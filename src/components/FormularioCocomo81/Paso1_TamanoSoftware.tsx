    ...
    <div className="h-full flex flex-col justify-between">
      <div className="overflow-y-auto pr-1 space-y-4">
        ...
        <div className="flex gap-2">
          <button
            className={`px-4 py-2 rounded-lg text-sm ${modo === "kloc" ? "bg-indigo-600 text-white" : "bg-gray-200"}`}
            onClick={() => setModo("kloc")}
          >
            Ingresar KLOC
          </button>
          <button
            className={`px-4 py-2 rounded-lg text-sm ${modo === "pf" ? "bg-indigo-600 text-white" : "bg-gray-200"}`}
            onClick={() => setModo("pf")}
          >
            Usar Puntos de Función
          </button>
        </div>
        ...
        {modo === "pf" && (
          <div className="space-y-4">
            {/* Componentes de PF */}
            <div>
              <h4 className="font-medium text-sm text-indigo-700 mb-1">Componentes de Función</h4>
      ...
      {["baja", "media", "alta"].map((nivel) => (
        <div key={nivel} className="flex items-center gap-2">
          ...
          />
            ...
            <div className="pt-6">
  {(() => {
    const { PFSA, PFA, KLOC } = calcularTodo();
    return (
      ...
      <div className="h-5 flex items-center justify-between mt-4 px-2 bg-white">
        ...
        <button
          onClick={() => onNext(modo === "kloc" ? kloc : calcularTodo().KLOC)}
...
export default Paso1_TamanoSoftware;
